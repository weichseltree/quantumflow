...
def calculate_density_and_energies(potential, wavefunctions, energies, N, h):
    ...
    kinetic_energy_densities = (
        0.5 * quantumflow.utils.derivative_five_point(wavefunctions, h) ** 2
    )  # == -0.5*wavefunctions*laplace_five_point(wavefunctions)
...
def calculate_system_properties(potential, wavefunctions, energies, N, h):
    ...
    (
        ...
        potential_energy_density,
        kinetic_energy_density,
    ) = calculate_density_and_energies(potential, wavefunctions, energies, N, h)
    ...
    vW_derivative = np.concatenate(
        [vW_derivative[:, 0:1], vW_derivative, vW_derivative[:, -1:]], axis=1
    )
...
class DensityKineticEnergyDataset(quantumflow.Dataset):
    ...
    def build(self, force=False):
        ...
        if self.experiments_dir is None:
            ...
            dataset_base_dir = os.path.join(self.experiments_dir, self.experiment)
        dataset_params = quantumflow.utils.load_yaml(
            os.path.join(dataset_base_dir, f"{self.experiment}.yaml")
        )[self.run_name]
        ...
        if self.N == "all":
            all_data = [
                calculate_system_properties(potential, wavefunctions, energies, N, h)
                for N in range(1, wavefunctions.shape[2] + 1)
            ]
            (
                density,
                energy,
                ...
                np.concatenate([all_data[i][j] for i in range(len(all_data))], axis=0)
        ...
        if self.dtype == "double" or self.dtype == "float64":
            if potential.dtype == np.float32:
                raise ImportError(
                    "requested dtype={}, but dataset is saved with dtype={}, which is less precise.".format(
                        params["dtype"], potential.dtype
                    )
                )
    ...
    def visualize(self, preview=5, figsize=(20, 3), dpi=None):
        ...
        plt.figure(figsize=figsize, dpi=dpi)
        plt.plot(
            self.x, np.transpose(self.kinetic_energy_density)[:, :preview]
        )  # only plot first potentials
