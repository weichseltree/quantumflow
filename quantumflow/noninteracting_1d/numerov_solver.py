...
def shooting_numerov(k_squared, h, numerov_init_slope, dtype):
    ...
    outputs = tf.keras.layers.RNN(
        ShootingNumerovCell(k_squared.shape[2:], h), return_sequences=True, dtype=dtype
    )(k_squared[:, 2:], initial_state=init_state)
    output = tf.concat(
        [tf.expand_dims(init_values, axis=1), tf.expand_dims(one_step_values, axis=1), outputs],
        axis=1,
    )
    return output
...
def numerov_k_squared(potentials, energies):
    return 2 * (
        tf.expand_dims(energies, axis=1)
        - tf.repeat(tf.expand_dims(potentials, axis=2), energies.shape[1], axis=2)
    )
...
def find_split_energies(potentials, N, h, dtype, numerov_init_slope):
    ...
    solutions_split = tf.zeros(
        (potentials.shape[0], potentials.shape[1], N + 1), dtype=potentials.dtype
    )
    ...
    while tf.math.reduce_any(not_converged):
        V_split = numerov_k_squared(
            tf.boolean_mask(potentials, not_converged), tf.boolean_mask(E_split, not_converged)
        )

        solutions_split_new = shooting_numerov(V_split, h, numerov_init_slope, dtype)

        partitioned_data = tf.dynamic_partition(
            solutions_split, tf.cast(not_converged, tf.int32), 2
        )
        condition_indices = tf.dynamic_partition(
            tf.range(tf.shape(solutions_split)[0]), tf.cast(not_converged, tf.int32), 2
        )

        solutions_split = tf.dynamic_stitch(
            condition_indices, [partitioned_data[0], solutions_split_new]
        )
        ...
        not_converged = tf.logical_and(
            tf.logical_not(tf.reduce_all(tf.equal(roots_split, target_roots), axis=1)),
            not_converged,
        )

        search_direction = tf.cast(roots_split < target_roots, potentials.dtype) - tf.cast(
            roots_split > target_roots, potentials.dtype
        )
...
def solve_numerov(potentials, target_roots, split_energies, h, numerov_init_slope, dtype):
    ...
    solutions_low = shooting_numerov(
        numerov_k_squared(potentials, E_low), h, numerov_init_slope, dtype
    )
    roots_low = tf.cast(detect_roots(solutions_low), tf.double)

    solutions_high = shooting_numerov(
        numerov_k_squared(potentials, E_high), h, numerov_init_slope, dtype
    )
...
def solve_schroedinger(potentials, N, h, dtype, numerov_init_slope):
    ...
    solutions_forward, E_forward, invalid_forward = solve_numerov(
        potentials, target_roots, E_split, h, numerov_init_slope, dtype
    )
    ...
    solutions_backward, E_backward, invalid_backward = solve_numerov(
        tf.reverse(potentials, axis=[1]), target_roots, E_split, h, numerov_init_slope, dtype
    )
    ...
    merge_value_forward = tf.reduce_sum(
        tf.gather(
            tf.transpose(solutions_forward, perm=[0, 2, 1]),
            tf.expand_dims(merge_index, axis=2),
            batch_dims=2,
        ),
        axis=2,
    )
    merge_value_backward = tf.reduce_sum(
        tf.gather(
            tf.transpose(solutions_backward, perm=[0, 2, 1]),
            tf.expand_dims(merge_index, axis=2),
            batch_dims=2,
        ),
        axis=2,
    ...
    join_mask = tf.expand_dims(tf.expand_dims(tf.range(G), axis=0), axis=2) < tf.expand_dims(
        merge_index, axis=1
    )
