...
def run_sweep(
    ...
    print("=" * 70)
    header = (
        f"{'Beta':<10} | {'Sliced W2 (50 steps)':<20} | "
        f"{'Empirical W2':<15} | {'Mean Eig Spread':<18}"
    )
    print(header)
    print("-" * len(header))
    for beta_str, r in sweep_results.items():
        sliced = r["sliced_wasserstein_distance"]
        empirical = r["empirical_wasserstein_distance"]
        eig_spr = r["mean_eigenvalue_spread"]
        print(f"{beta_str:<10} | {sliced:<20.5f} | {empirical:<15.5f} | {eig_spr:<18.5f}")
    print("=" * 70)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/transport"),
        help="Base output directory",
    )
    parser.add_argument("--seed", type=int, default=42, help="Seed shared by every beta run")
