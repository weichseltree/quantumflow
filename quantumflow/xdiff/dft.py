...
class XLayer(tf.keras.layers.Layer):
    def __init__(self, dataset, subsample_inputs=1, **kwargs):
        ...
        self.x = tf.Variable(
            initial_value=np.float32(dataset.x[np.newaxis, :, np.newaxis]),
            trainable=False,
            name="x",
        )
        self.x_inputs = tf.Variable(
            initial_value=np.float32(
                dataset.x[np.newaxis, np.newaxis, ::subsample_inputs, np.newaxis]
            ),
            trainable=False,
            name="x_inputs",
        )
    ...
    def call(self, density):
        return (
            ...
            tf.repeat(
                tf.repeat(self.x_inputs, self.x.shape[1], axis=1), tf.shape(density)[0], axis=0
            ),
            tf.ensure_shape(
                tf.repeat(
                    tf.expand_dims(
                        tf.expand_dims(density[:, :: self.subsample_inputs], axis=-1), axis=1
                    ),
                    self.x.shape[1],
                    axis=1,
                ),
                (None, self.x.shape[1], self.x.shape[1] // self.subsample_inputs, 1),
            ),
        )
...
def XdiffTransformer_KineticEnergyDensityFunctional(
    run_dir, dataset, subsample_inputs=1, loop=False, **kwargs
):
...
def XdiffPerciever_KineticEnergyDensityFunctional(run_dir, dataset, **kwargs):
    ...
    kinetic_energy_density = tf.keras.layers.Lambda(
        lambda x: tf.reduce_sum(x[..., 0], axis=-1), name="kinetic_energy_density"
    )(value)
    kinetic_energy = IntegrateLayer(dataset.h, name="kinetic_energy")(kinetic_energy_density)
