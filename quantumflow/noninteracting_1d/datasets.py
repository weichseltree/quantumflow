...
def generate_potentials(
    ...
    **kwargs,
    ...
    a = tf.random.uniform(
        (dataset_size, 1, n_gauss), minval=a_minmax[0], maxval=a_minmax[1], dtype=dtype, name="a"
    )
    b = tf.random.uniform(
        (dataset_size, 1, n_gauss),
        minval=b_minmax[0] * interval_length,
        maxval=b_minmax[1] * interval_length,
        dtype=dtype,
        name="b",
    )
    c = tf.random.uniform(
        (dataset_size, 1, n_gauss),
        minval=c_minmax[0] * interval_length,
        maxval=c_minmax[1] * interval_length,
        dtype=dtype,
        name="c",
    )
    ...
    if return_h:
        h = tf.cast(
            interval_length / (discretisation_points - 1), dtype=dtype
...
class PotentialDataset(quantumflow.Dataset):
    ...
    def build(self):
        self.potential, self.x, self.h = generate_potentials(
            return_x=True, return_h=True, **self.kwargs
        )
...
class Non1D_QFDataset(quantumflow.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.potential_generator = quantumflow.instantiate(
            potentials, dataset_size=self.dataset_size, dtype=self.dtype, **kwargs
        )
    ...
    def build(self, force=False):
        if force or not os.path.isfile(os.path.join(self.run_dir, self.filename)):
            ...
            potential, x, h = (
                self.potential_generator.potential,
                self.potential_generator.x,
                self.potential_generator.h,
            )
            ...
            if self.dataset_size % self.generate_batch_size:
                energies, wavefunctions = quantumflow.noninteracting_1d.solve_schroedinger(
                    potential[
                        (self.dataset_size // self.generate_batch_size) * self.generate_batch_size :
                    ],
                    self.N,
                    h,
                    self.dtype,
            ...
            self.x, self.h, self.potential, self.wavefunctions, self.energies = load_dataset(
                self.run_dir, self.filename
            )
    ...
    def visualize(self, preview=5, figsize=(20, 3), dpi=None):
        ...
        for i, plot in enumerate(
            self.wavefunctions[:preview] ** 2 + self.energies[:preview, np.newaxis, :]
        ):
            for n, plot_single in enumerate(plot.transpose()):
                ...
                axs[i].set_ylim(
                    [
                        np.min(self.potential[:preview]),
                        max(np.max(self.energies[:preview] * 1.1), 0.5),
                    ]
                )
                if i == 0:
        ...
        for i in range(self.energies.shape[1]):
            ...
            plt.hist(
                ...
                bins,
                alpha=0.7,
                color=[0.8 * color**2, 0.8 * (1 - color) ** 2, 1.6 * color * (1 - color)],
                label=f"{i}-th solution",
            )
