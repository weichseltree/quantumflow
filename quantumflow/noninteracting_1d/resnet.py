...
class IntegrateLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        return self.h * tf.reduce_sum(
            (inputs[:, :-1] + inputs[:, 1:]) / 2.0, axis=1, name="trapezoidal_integral_approx"
        )
...
def ResNet_KineticEnergyDensityFunctional(
    run_dir, dataset, blocks, l2_regularisation=0.0, **kwargs
):
    kernel_regularizer = (
        tf.keras.regularizers.l2(l2_regularisation) if l2_regularisation > 0.0 else None
    )
    ...
    def resnet_block(input, filters, kernel_size, padding=None, activation=None, add_input=True):
        ...
        return (
            tf.keras.layers.Activation(activation=activation)(value)
    ...
    kinetic_energy_density = tf.keras.layers.Lambda(
        lambda x: tf.reduce_sum(x, axis=-1), name="kinetic_energy_density"
    )(value)
...
def FixupResNet_KineticEnergyDensityFunctional(
    run_dir, dataset, blocks, final_layer, l2_regularisation=0.0, fixup_m=3, **kwargs
):
    kernel_regularizer = (
        tf.keras.regularizers.l2(l2_regularisation) if l2_regularisation > 0.0 else None
    )
    ...
    def fixup_bottleneck_block(
        input, filters, kernel_size, padding=None, activation=None, add_input=True
    ):
        value = input
        ...
        for layer in range(len(filters)):
            if layer < len(filters) - 1:
                kernel_initializer = tf.keras.initializers.VarianceScaling(
                    scale=2 * fixup_scale_variance, mode="fan_in", distribution="truncated_normal"
                )
            else:
