...
def integrate(y, h):
    return h * tf.reduce_sum(
        (y[:, :-1] + y[:, 1:]) / 2.0, axis=1, name="trapezoidal_integral_approx"
    )


def np_integrate(integrand, h, axis=1):
    return h * (
        np.sum(integrand, axis=axis)
        - 0.5 * (np.take(integrand, 0, axis=axis) + np.take(integrand, -1, axis=axis))
    )
    ...
    return tf.concat(
        [
            ...
            * (
            ...
            1
            / (12 * h)
            * (value[:, 0:-4] - 8 * value[:, 1:-3] + 8 * value[:, 3:-1] - value[:, 4:]),
            1
            / (12 * h)
            * (
                -value[:, -5:-4]
...
def laplace_five_point(value, h):
    return tf.concat(
        [
            1
            / (h**2)
            * (2 * value[:, 0:1] - 5 * value[:, 1:2] + 4 * value[:, 2:3] - 1 * value[:, 3:4]),
            1
            / (12 * h**2)
            * (
                11 * value[:, 0:1]
            ...
            * (
            ...
            1
            / (h**2)
            * (2 * value[:, -1:] - 5 * value[:, -2:-1] + 4 * value[:, -3:-2] - 1 * value[:, -4:-3]),
        ],
        axis=1,
    )
