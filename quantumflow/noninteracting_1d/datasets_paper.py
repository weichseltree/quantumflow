...
def calculate_potentials(
    a, b, c, discretisation_points=500, interval_length=1.0, dtype="float64", **kwargs
):
...
class TXTPotentialDataset(quantumflow.Dataset):
    ...
    def build(self):
        paper_coeff = pd.read_csv(
            os.path.abspath(os.path.join(self.run_dir, self.filename)), delimiter=" "
        )

        a = paper_coeff[["a1", "a2", "a3"]].values[:, np.newaxis, :]
        c = paper_coeff[["b1", "b2", "b3"]].values[
            :, np.newaxis, :
        ]  # b<->c listed wrong in the paper
